...
function LeadTimeCalendar({
  ...
  const derivedDates = useMemo(() => {
    ...
    return {
      ...
      expectedDisplay: formatDate(expectedDate),
    }
    ...
    <div className="mt-4 rounded-3xl border border-primary/15 bg-gradient-to-b from-card to-background px-4 py-4 shadow-sm">
      ...
      {!keyword.trim() || expectedDays == null ? (
        ...
        >
          ...
          {days.map((day) => {
            ...
            const isDisabled = !keyword.trim() || expectedDays == null
            ...
            return (
              <button
                ...
                className={cn(
                  "h-14 rounded-xl border border-transparent bg-card/60 px-1 py-1 shadow-[inset_0_0_0_1px_rgba(59,91,219,0.06)] transition-colors",
      ...
      {derivedDates && (
        <div className="mt-3 rounded-lg border border-border/60 bg-background px-3 py-2 text-xs text-muted-foreground">
          선택 날짜: {derivedDates.selectedDisplay} · 예상 결과 발표일: {derivedDates.expectedDisplay}
        </div>
      )}
    </div>
  )
      ...
      <SheetContent side="bottom" className="h-[85dvh] rounded-t-2xl p-0">
        ...
        <div className="overflow-y-auto p-6 space-y-6">
          <section className="rounded-2xl border border-border/60 bg-card p-4">
            <h3 className="text-sm font-medium text-muted-foreground mb-1">회사 타임라인</h3>
            ...
            {isTimelineLoading ? (
              ...
              <div className="space-y-6">
                {timeline.timelines.map((unitTimeline, unitIndex) => (
                  <div key={`${unitTimeline.unitName}-${unitTimeline.channelType}-${unitTimeline.year}-${unitIndex}`}>
                    ...
                    <div className="rounded-2xl border border-border/60 bg-background px-3 py-4">
                      <div className="relative">
                        ...
                        {unitTimeline.steps.map((step, index) => {
                          const label = step.label?.trim() || "전형 진행"
          ...
          <section className="rounded-2xl border border-border/60 bg-card p-4">
            <h3 className="text-sm font-medium text-muted-foreground mb-1">전형 기간 분석</h3>
            ...
            <form onSubmit={onKeywordSearch} className="flex gap-2">
              <Input
                ...
                placeholder="전형 키워드를 입력 (예: 1차면접, 코딩 테스트)"
                className="h-10 rounded-xl border-border bg-card"
              />
              <button
                type="submit"
                disabled={isLeadTimeLoading}
                className="h-10 min-w-[72px] px-4 rounded-xl bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors disabled:opacity-50"
              >
                {isLeadTimeLoading ? "분석 중..." : "검색"}
              </button>
            </form>
